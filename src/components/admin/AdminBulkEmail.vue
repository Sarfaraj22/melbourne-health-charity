...
const sendAll = ref<boolean>(true)
...
const selectedCount = computed<number>(() =>
  sendAll.value ? props.volunteers.length : selectedIds.value.size,
)
...
async function handleSubmit(): Promise<void> {
  ...
  if (!sendAll.value && selectedIds.value.size === 0) {
    status.value = 'error'
    errorMessage.value = 'Select at least one volunteer, or send to all.'
    return
  }
  try {
    const result = await sendBulkEmail({
      ...
      volunteerIds: sendAll.value ? [] : [...selectedIds.value],
    })
...
<template>
  <section class="bg-surface px-5 py-6 sm:px-8" aria-labelledby="admin-bulk-email-heading">
    <div class="mx-auto flex max-w-container flex-col gap-4">
      <h2 id="admin-bulk-email-heading" class="text-2xl font-bold text-text-default">
        Email volunteers
      </h2>
      <form class="flex max-w-2xl flex-col gap-4" novalidate @submit.prevent="handleSubmit">
        <label class="flex items-center gap-2 text-sm text-text-default">
          <input v-model="sendAll" type="checkbox" class="accent-brand-primary" />
          Send to all volunteers ({{ volunteers.length }})
        </label>
        <fieldset v-if="!sendAll" class="flex flex-col gap-2">
        ...
        <div class="flex flex-col gap-1.5">
          <label for="bulk-email-subject" class="text-sm font-medium text-text-default">
            Subject
          </label>
          <input
        ...
        <p v-if="status === 'success'" class="text-sm text-status-success" role="status">
          Sent to {{ sentCount }} volunteer{{ sentCount === 1 ? '' : 's' }}.
        </p>
